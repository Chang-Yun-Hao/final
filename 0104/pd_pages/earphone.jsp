    ...
    try{
        ...
        if(con.isClosed())
        ...
        else
          {
            ...
            String sqlP = "SELECT * FROM product WHERE pdKind = 'earphone' GROUP BY pdKind";
            ...
            String sqlS1 = "SELECT * FROM product WHERE pdId = '49' ";
  ...
  <div class="content">
    <div class="insideleft">
      <div>
        ...
        <div title="點擊預覽">
        <div>
          <img onclick="showBgImg(this)" class="thum-img" src="../images/products/see/EX/earphone750.png">
        </div>
      ...
      <div>
        ...
        <p>
          <div class="word">款式：
            <a href="#img1"> <button name=" BK-S1" class="button_w " disabled>BK-S1藍牙耳機</button></a>
          </div>
        </p>
 <script>
function changeId(i)
        ...
        <p>
          ...
          <div class="box1">
            <div class="note_w">
              ...
              全球獨步混音技術(Audio Mixer)
              對講&A2DP混音技術
              當您使用手機享受音樂/手機導航音訊時，在前後對講通話中，前後座都聽到各自手機的音訊的來源。
              GPS混音技術
              當您使用Garmin 660導航機時，在前後對講通話中，前座&後座都可以聽到Garmin 660導航機。
              手機、GPS混音技術
              當同時連接手機和Garmin 660時，在手機通話中，可以同時接收來自GPS路況訊息。
              音樂共享
              可將您手機裡的藍芽立體聲音樂透過分享音樂給您的朋友。
              麥克風 / 聲音品質(抗噪音)
              聲音經過麥克風後,經過硬體的濾波過濾基本上就可以消除低頻聲(如風切聲),再透過軟體上使用DSP數位信號處,更加一步過濾並消除一些非人為的聲音(如引擎轉速聲)。透過這兩步驟讓本公司產品所提供的音質更加完美,特殊的抗風切設計,使得聲音在高速下仍然清晰。

            </div>
            <p>
              <img class="notebox" src="../images/products/EX/earphone1.png.jpg">
            </p>
          </div>
      ...
      <div class="insideleft">
        ...
        <input type="hidden" name="PDID" value="<%=ID%>">  <!--傳到後端-->
          <fieldset>
            ...
            <br>
            <button name="output" type="submit" class="button_w">留言</button>
           </form>
          </fieldset>
          <hr>

 <%
        String sqlm = "SELECT message.msgId, member.memName, message.msgContent, message.msgDate FROM message JOIN `member` ON message.memId = member.memId WHERE pdId = '49' ORDER BY message.msgId DESC;";
